    ...
    for (int row = 0; row < Board::HEIGHT; row++) {
        for (int column = 0; column < Board::WIDTH; column++) {
            ...
            switch (piece) {
                case -1:
                case 1:
                    new_moves = GeneratePawnMoves(board, column, row);
                    break;
                case -2:
                case 2:
                    new_moves = GenerateKnightMoves(board, column, row);
                    break;
                case -3:
                case 3:
                    new_moves = GenerateBishopMoves(board, column, row);
                    break;
                case -4:
                case 4:
                    new_moves = GenerateRookMoves(board, column, row);
                    break;
                case -5:
                case 5:
                    new_moves = GenerateQueenMoves(board, column, row);
                    break;
                case -6:
                case 6:
                    new_moves = GenerateKingMoves(board, column, row);
                    break;
                default:
                    break;
            }
...
std::vector< std::string> MoveGenerator::GeneratePawnMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    std::string move = "";
...
std::vector<std::string> MoveGenerator::GenerateKnightMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    bool is_white = board.GetPieceAt(column, row) > 0;
    std::string move = "";
    std::string  cur_pos = board.PositionToString(column, row);
    ...
    for (int row_step = -2; row_step <= 2; row_step++) {
        ...
        move = cur_pos + board.PositionToString(next_column, next_row);
        if (board.IsCapture(next_column, next_row, is_white) || board.IsFree(next_column, next_row)) {
            moves.push_back(move);
        }
        // right
        next_column = column +column_step;
...
std::vector<std::string> MoveGenerator::GenerateBishopMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    bool is_white = board.GetPieceAt(column, row) > 0;
    std::string move = "";
    std::string cur_pos = board.PositionToString(column, row);
        ...
        for (int column_step = -1; column_step <= 1; column_step += 2) {
            ...
            while (board.IsValidPos(next_column, next_row)) {
                ...
                if (board.IsFree(next_column, next_row)) {
                  ...
                  // If occupied by enemy add to list before break
                  if (board.IsCapture(next_column, next_row, is_white)) {
                    moves.push_back(move);
                  }
                  break;
...
std::vector<std::string> MoveGenerator::GenerateRookMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    bool is_white = board.GetPieceAt(column, row) > 0;
    std::string move = "";
    std::string cur_pos = board.PositionToString(column, row);
...
std::vector<std::string> MoveGenerator::GenerateQueenMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    moves = GenerateBishopMoves(board, column, row);
    std::vector<std::string> rookMoves = GenerateRookMoves(board, column, row);
    moves.insert(moves.end(), rookMoves.begin(), rookMoves.end());
    return moves;
}

std::vector<std::string> MoveGenerator::GenerateKingMoves(Board& board, int column, int row) {
    std::vector<std::string> moves;
    bool is_white = board.GetPieceAt(column, row) > 0;
    std::string move = "";
    std::string cur_pos = board.PositionToString(column, row);
    ...
    for (int next_row = row-1; next_row <= row+1; next_row++) {
        ...
        for (int next_column = column-1; next_column <= column+1; next_column++) {
            ...
            // Free
            if (board.IsFree(next_column, next_row) || board.IsCapture(next_column, next_row, is_white)) {
                moves.push_back(move);
            }
        }
    }
...
bool MoveGenerator::KingIsSafe(Board& board, std::string move) {
    ...
    moves = GenerateKnightMoves(board, king_column, king_row);
    for (size_t i = 0; i < moves.size(); i++) {
    ...
    // Generate Rook moves
    moves = GenerateRookMoves(board, king_column, king_row);
    for (size_t i = 0; i < moves.size(); i++) {
    ...
    moves = GenerateBishopMoves(board, king_column, king_row);
    for (size_t i = 0; i < moves.size(); i++) {
    ...
    // Generate Pawn moves
    moves = GeneratePawnMoves(board, king_column, king_row);
    for (size_t i = 0; i < moves.size(); i++) {
    ...
    // Generate King moves
    moves = GenerateKingMoves(board, king_column, king_row);
    for (size_t i = 0; i < moves.size(); i++) {
